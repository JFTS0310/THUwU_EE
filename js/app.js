...
function updateCreditsUI() {
    ...
    document.querySelector("#credits-count").textContent = `${total} 學分`;
...
const settingOptions = [
    ...
    {
        key: "hideTag",
        description: "隱藏課程列表中的tag",
        callback: (value) => {
            value ? document.body.classList.add('hide-tags') : document.body.classList.remove('hide-tags');
        }
    },
    ...
    filterHeader.onclick = () => {
        ...
        if (isHidden) {
            ...
            requestAnimationFrame(() => requestAnimationFrame(() => {
                filterContent.style.transition = 'opacity 0.3s ease, max-height 0.35s ease';
                filterContent.style.opacity = '1';
                filterContent.style.maxHeight = '600px';
            }));
            setTimeout(() => {
...
function renderConfig(options) {
    ...
    options.forEach(rule => {
        ...
        checkbox.onclick = ({ target }) => {
            ...
            rule.callback(target.checked, false);
        };
        ...
        rule.callback(checkbox.checked, true);
    });
    config = storedConfig;
}
